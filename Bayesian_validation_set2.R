###################################################################################
### CODES TO RUN THE BAYESIAN VALIDATION FRAMEWORK (SETTING 2)
###################################################################################
...
bias.pct = as.character(commandArgs(trailingOnly = T)[1]) #Bias pct introduced in model outputs to create the faulty structure scenario; 0 in this case (parallel script argument 1)
which.theta.bias = as.integer(commandArgs(trailingOnly = T)[2]) #Which parameter has the bias (parallel script argument 2)
theta.bias.pct = as.integer(commandArgs(trailingOnly = T)[3]) #Percentage of bias in the parameter (parallel script argument 3)
update.theta = as.logical(commandArgs(trailingOnly = T)[4]) #whether to update priors on parameters  (parallel script argument 4)
script = as.integer(commandArgs(trailingOnly = T)[5]) #to run parallel with different seeds (parallel script argument 5)
bias = gsub("[.]", "", bias.pct)
...
### FOR FLAT PRIOR  
delta.prec = 0.1 #precision parameter for controlling the variance of the parameters logitgauss or uniform prior
### FOR NARROW PRIOR
...
### FOR FLAT PRIOR    
c_beta = 30 #controls the parameters of the beta distribution proposal for the parameters
### FOR NARROW PRIOR  
...
## NEW STEP: Adding bias to theta (determined by which.theta.bias)
