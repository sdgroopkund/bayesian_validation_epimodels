...
data.generate<-function(nloc,prev13,u,time.list,Interventioninput,sample.size,bias.pct){
  ...
  ##!!!!!SCALE THE POPULATION WHEN YOU TURN THE INTERVENTION ON
  #need to get the pop size for the start year of the intervention 2015
  #year 35 in the model (time step)
  #Modify the InitialPopSize- now needs to be location specific after you have made this adjustment
  ...
  for(c in 1:nloc){
    parmsfit <-c(notatrisk=parameters[1,c],ART=0,scale=1,circmax=circatbirth,inttime=(MaxTime+10),propcircbirth=circatbirth,betafit=beta,trend=trenduncertain)#intervention is not on
    xstart <- c(S1=(parameters[1,c]*circatbirth*(InitialPopSize*(1-InitialPropInfect))),S2=((1-parameters[1,c])*circatbirth*(InitialPopSize*(1-InitialPropInfect))),S3=((1-parameters[1,c])*(1-circatbirth)*(InitialPopSize*(1-InitialPropInfect))),S4=(parameters[1,c]*(1-circatbirth)*(InitialPopSize*(1-InitialPropInfect))),I1=(InitialPopSize*InitialPropInfect*0.5),I2=(InitialPopSize*InitialPropInfect*0.5),AI=0,NAI=0) 
...
MCMC.ana.mh<-function(prev13,yF,mod.output,u.m,mub,gammab,betab,time.list,sample.size,scale1,scale2,c_beta,c_invgam=NULL,
                      truncated.norm=T,sequential=T,delta=NULL,delta.prec=1e-2,use.X=T,int.on=T,Interventioninput=c(0.10,0.05,0.15),
  ...
  if (use.X==T){
    ...
    Z=cbind(prev13,matrix(rep(u.0,each=length(prev13)),nrow=length(prev13)))
    R<-corr_matrix(Z,betab, corr=list(type='exponential',power=2))
    R.fixed=NULL
  }
  Sig_b<-lb.inv.0*R
...
metro.hast <- function(theta,lik.fun,prior.fun,scale1,scale2,c_beta,u.m,delta.prec,delta,c_invgam,truncated.norm,mcmc.sample,burn.in,thin.in,sequential,prior,R.fixed,gammab,mub,betab,sample.size,nloc,prev13,time.list,Interventioninput,int.on) {
  prior.u<-get(prior)
  if (! sequential) {
...
metro.hast.notheta <- function(theta,lik.fun,prior.fun,scale1,u.m,delta.prec,delta,c_invgam,mcmc.sample,burn.in,thin.in,sequential,prior,R.fixed,gammab,mub,betab,sample.size,nloc,prev13,time.list,Interventioninput,int.on) {
  prior.u<-get(prior)
  if (! sequential) {
    if (! require(mvtnorm)) stop("You need to install the mvtnorm library.")
  }
...
proposal.loglike.uni.notheta<-function(i,theta1,theta2,scale1,c_invgam,u.m,prev13){
  if (i %in% (length(u.m)+1):(length(u.m)+length(prev13))){
    ...
    qratio.proposal<-log(dinvgamma(theta1,shape=c_invgam+1,scale=c_invgam*theta2))-log(dinvgamma(theta2,shape=c_invgam+1,scale=c_invgam*theta1))
  }
...
proposal.loglike.mult<-function(theta1,theta2,scale1,c_beta,scale2,c_invgam,truncated.norm){
  ...
  loglike.lb<-log(dinvgamma(theta1.lb,shape=c_invgam+1,scale=c_invgam*theta2.lb))-log(dinvgamma(theta2.lb,shape=c_invgam+1,scale=c_invgam*theta1.lb))
...
## The functions below are for a more generalized setting; the above functions were used in Settings 1 and 3
## Priors only on some theta
## the subtheta versions of the functions are for setting 2: faulty theta
...
MCMC.ana.mh.subtheta<-function(prev13,yF,mod.output,u.m,mub,gammab,betab,time.list,sample.size,scale1,scale2,
                               ...
                               use.X=T,int.on=T,Interventioninput=c(0.10,0.05,0.15),MCMC.samples=5000,burn.in=2000,
...
loglike.prior.subtheta<-function(par){
  ...
  ### likelihood
  if (prior=='unif.prior'){
    if (is.null(delta)==T) delta<-u.m[thetaind]*(1-u.m[thetaind])*(u.m[thetaind]<0.9 & u.m[thetaind]>0.1) + pmin(1-u.m[thetaind],u.m[thetaind])*(u.m[thetaind]>0.9 |u.m[thetaind]<0.1)
